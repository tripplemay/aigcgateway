/**
 * Retry the 17 models that failed in the non-streaming run, using stream=true.
 * Streaming keeps SSE bytes flowing, so Nginx's 120s proxy_read_timeout never
 * triggers (timeout resets on each chunk).
 *
 * Stagger 400ms between requests to stay safely under the 20-req/5s burst
...
async function callStreaming(model: string): Promise<Result> {
  ...
  try {
    ...
    console.log(
      ...
      `out=${usage?.completion_tokens ?? "?"}  chars=${content.length}`,
    );
    return { model, durationMs: ms, ttftMs: ttft, ok: true, content, usage };
  } catch (err: any) {
    clearTimeout(timer);
    const ms = Date.now() - start;
...
async function main() {
  ...
  console.log(`Streaming retry: ${MODELS.length} models, stagger ${STAGGER_MS}ms\n`);
  ...
  console.log("Full answers: scripts/benchmark-results-stream.json");
}
