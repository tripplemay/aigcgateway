...
export const OR_IMAGE_PRICES: readonly OrChannelPriceEntry[] = [
  { channelId: "cmnpqumpb008zbnxc2t47ollt", model: "google/gemini-2.5-flash-image", inputPer1M: 0.3, outputPer1M: 2.5 },
  { channelId: "cmnpqumjc006wbnxceftbpqv3", model: "google/gemini-3-pro-image-preview", inputPer1M: 2.0, outputPer1M: 12.0 },
  { channelId: "cmnpqum5m002bbnxcr4b4v3ew", model: "google/gemini-3.1-flash-image-preview", inputPer1M: 0.5, outputPer1M: 3.0 },
  { channelId: "cmnpqumo4008kbnxck2puju4i", model: "openai/gpt-5-image", inputPer1M: 10.0, outputPer1M: 10.0 },
  { channelId: "cmnpqumn40088bnxcn4z62t2x", model: "openai/gpt-5-image-mini", inputPer1M: 2.5, outputPer1M: 2.0 },
  { channelId: "cmo9iyi2w0buxbnvxe4c1aaqt", model: "openai/gpt-5.4-image-2", inputPer1M: 8.0, outputPer1M: 15.0 },
];
...
export async function runOrPricingFix(opts: RunOptions): Promise<{
  ...
  for (const entry of OR_IMAGE_PRICES) {
    ...
    console.log(
      `[${opts.apply ? "apply " : "dry-run"}] ${entry.model} (${entry.channelId})`,
    );
...
async function cliMain(): Promise<void> {
  ...
  console.log(
    `=== F-BIPOR-01 OR image channel pricing fix (${apply ? "APPLY" : "DRY-RUN"}) ===`,
  );
  if (apply) {
    console.log(
