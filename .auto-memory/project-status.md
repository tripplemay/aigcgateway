...
- Status: `fixing`（reverifying round6 仍失败，继续修复）
- 诊断结论：常规点击 Save Changes 时“无 toast + 无 PATCH”；脚本触发 save-profile-btn click 时“有 PATCH + 持久化”
- 关键含义：后端更新链路可通，但常规点击路径仍不稳定；本轮代码里 `toast.info("Saving name...")` 信号已不存在
- 报告：`docs/test-reports/R2B-user-pages-balance-usage-settings-reverifying-round6-2026-04-09.md`
- 背景：generator round6 改为 `button + 原生 addEventListener('click')`，仍待修复常规点击路径
