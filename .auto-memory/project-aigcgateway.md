...
**当前批次：** `admin-model-capabilities`（verifying 结果 4 PASS / 1 FAIL，已回退 fixing）
**失败项：** `F-MC-07` AC5 — `/v1/models` 中 `openai/gpt-4o` 的 `capabilities` 为空 `{}`（证据：`docs/test-reports/admin-model-capabilities-e2e-2026-04-08.json`）
