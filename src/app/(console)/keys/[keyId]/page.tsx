...
export default function KeySettingsPage() {
  ...
  const { data: detail, loading } = useAsyncData<KeyDetail | null>(
    async () => {
      if (!current || !params.keyId) return null;
      const r = await apiFetch<{ data: KeyDetail }>(`/api/projects/${current.id}/keys/${params.keyId}`);
      const d = r.data;
      setName(d.name ?? "");
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-ds-surface-container-lowest rounded-xl p-6 shadow-sm ring-1 ring-ds-primary/5">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-ds-on-surface-variant/60 mb-2">{t("keyName")}</label>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-ds-on-surface-variant/60 mb-2">{t("description")}</label>
            ...
            <section className="bg-ds-surface-container-lowest rounded-xl p-6 shadow-sm ring-1 ring-ds-primary/5">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-[var(--font-heading)] font-bold text-lg">{t("permissions")}</h3>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {PERM_KEYS.map((key) => {
                  ...
                  return (
                    <div key={key} className={`flex items-center justify-between p-4 bg-ds-surface-container-low rounded-xl hover:bg-ds-surface-container-high transition-colors ${!enabled ? "opacity-60" : ""}`}>
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-ds-on-surface">{t(key)}</span>
                        <span className="text-[11px] text-ds-on-surface-variant/70">{t(`${key}Desc`)}</span>
                      </div>
                      <button onClick={() => !isRevoked && togglePerm(key)} disabled={isRevoked} className={`w-11 h-6 rounded-full relative transition-colors ${enabled ? "bg-ds-primary" : "bg-ds-outline-variant"}`}>
            ...
            <section className="bg-ds-surface-container-lowest rounded-xl p-6 shadow-sm ring-1 ring-ds-primary/5">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h3 className="font-[var(--font-heading)] font-bold text-lg">{t("securityLimits")}</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-ds-on-surface-variant/60 mb-2">{t("rateLimit")}</label>
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-xs text-ds-on-surface-variant/60 font-semibold italic">{t("reqPerMin")}</span>
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-ds-on-surface-variant/60 mb-2">{t("ipWhitelist")}</label>
                  ...
                  <p className="mt-2 text-[10px] text-ds-on-surface-variant/50">{t("ipWhitelistHint")}</p>
                </div>
              </div>
            </section>
          </div>
          ...
          <div className="space-y-8">
            ...
            <section className="bg-ds-surface-container-lowest rounded-xl p-6 shadow-sm ring-1 ring-ds-primary/5">
              ...
              <div className={`flex items-center justify-between p-4 rounded-xl border ${isRevoked ? "bg-ds-error/5 border-ds-error/10" : "bg-ds-primary/5 border-ds-primary/10"}`}>
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] text-ds-on-surface-variant/70">{isRevoked ? t("permanentlyRevoked") : t("readyForRequests")}</span>
                </div>
              </div>
            </section>
            ...
            <section className="bg-ds-surface-container-lowest rounded-xl p-6 shadow-sm ring-1 ring-ds-primary/5">
              ...
              <div className="relative">
                ...
                <button onClick={() => { navigator.clipboard.writeText(detail.maskedKey); toast.success(tc("copied")); }} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-ds-surface-container-high rounded-md text-ds-primary transition-colors">
              ...
              <div className="mt-4 p-3 bg-ds-tertiary-fixed/30 rounded-lg flex gap-3">
                ...
                <p className="text-[11px] text-ds-on-surface-variant leading-snug">{t("neverShareKey")}</p>
              </div>
              {!isRevoked && (
            ...
            {!isRevoked && (
              <section className="bg-ds-surface-container-low rounded-xl p-6 ring-1 ring-ds-error/20">
                <div className="flex items-center gap-2 mb-4 text-ds-error">
                  ...
                  <h3 className="font-[var(--font-heading)] font-bold text-lg">{t("dangerZone")}</h3>
                </div>
                <p className="text-xs text-ds-on-surface-variant leading-relaxed mb-6">
                  {t("dangerZoneDesc")}
                </p>
                <button onClick={() => setRevokeOpen(true)} className="w-full py-3 px-4 border-2 border-ds-error/20 text-ds-error font-bold rounded-xl text-sm hover:bg-ds-error hover:text-white transition-all flex items-center justify-center gap-2">
