  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: t("active"), value: summary.active, color: "text-green-600" },
          { label: t("degraded"), value: summary.degraded, color: "text-yellow-600" },
          { label: t("disabled"), value: summary.disabled, color: "text-red-600" },
        ].map((c) => (
          <Card key={c.label}>
          ...
          <Card key={ch.channelId}>
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className={`w-3 h-3 rounded-full ${statusColor[ch.status] ?? "bg-gray-400"}`}
                />
                <div>
                  <div className="font-medium">
                    {ch.model} <span className="text-muted-foreground">{t("via")}</span>{" "}
                    {ch.provider}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {t("priority")} {ch.priority} | {ch.modality}
                  </div>
                </div>
              </div>
