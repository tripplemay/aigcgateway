...
interface UsageSummary { totalCalls: number; totalRevenue: number; totalCost: number; margin: number; avgLatencyMs: number; successRate: number }
...
export default function AdminUsagePage() {
  ...
  const load = async () => {
    ...
    setSummary(s); setByProvider(p.data); setByModel(m.data);
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Global Usage</h1>
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: "Revenue", value: formatCurrency(summary.totalRevenue, 2) },
            { label: "Cost", value: formatCurrency(summary.totalCost, 2) },
            { label: "Margin", value: `${formatCurrency(summary.margin, 2)} (${summary.totalRevenue > 0 ? ((summary.margin / summary.totalRevenue) * 100).toFixed(1) : 0}%)` },
          ].map((c) => (
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <Card><CardHeader><CardTitle className="text-sm">Revenue by Provider</CardTitle></CardHeader>
          <CardContent className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={byProvider} dataKey="revenue" nameKey="provider" cx="50%" cy="50%" outerRadius={80} label>
        ...
        <Card><CardHeader><CardTitle className="text-sm">Calls by Model</CardTitle></CardHeader>
          <CardContent className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={byModel.slice(0, 8)} layout="vertical">
                <XAxis type="number" /><YAxis type="category" dataKey="model" width={140} tick={{ fontSize: 11 }} />
                <Tooltip /><Bar dataKey="calls" fill="#2563eb" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <Card><CardHeader><CardTitle className="text-sm">Provider Cost Breakdown</CardTitle></CardHeader>
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow>
              <TableHead>Provider</TableHead><TableHead>Calls</TableHead><TableHead>Cost</TableHead>
              <TableHead>Revenue</TableHead><TableHead>Margin</TableHead><TableHead>Margin %</TableHead>
