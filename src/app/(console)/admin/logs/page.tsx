...
interface LogEntry {
  traceId: string; projectName: string; modelName: string; channelProvider: string;
  status: string; promptTokens: number | null; completionTokens: number | null;
  costPrice: number | null; sellPrice: number | null; latencyMs: number | null; createdAt: string;
}

const statusVariant: Record<string, "default" | "destructive" | "secondary" | "outline"> = {
  SUCCESS: "default", ERROR: "destructive", TIMEOUT: "destructive", FILTERED: "outline",
};
...
export default function AdminLogsPage() {
  ...
  const load = async () => {
    ...
    setLogs(r.data); setTotal(r.pagination?.total ?? r.data.length); setLoading(false);
  };
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <Input className="max-w-sm" placeholder="Search prompts, models, trace IDs..." value={searchQ} onChange={(e) => setSearchQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && doSearch()} />
      ...
      <Card><CardContent className="p-0">
        <Table>
          <TableHeader><TableRow>
            <TableHead>Time</TableHead><TableHead>Trace</TableHead><TableHead>Project</TableHead>
            <TableHead>Model</TableHead><TableHead>Provider</TableHead><TableHead>Status</TableHead>
            <TableHead>Tokens</TableHead><TableHead>Cost</TableHead><TableHead>Sell</TableHead><TableHead>Latency</TableHead>
          ...
          <TableBody>
            ...
            logs.map((l) => (
              <TableRow key={l.traceId}>
                ...
                <TableCell className="text-xs">{l.channelProvider}</TableCell>
                <TableCell><Badge variant={statusVariant[l.status] ?? "secondary"}>{l.status}</Badge></TableCell>
                <TableCell className="font-mono text-xs">{l.promptTokens ?? "—"}/{l.completionTokens ?? "—"}</TableCell>
                <TableCell className="font-mono text-xs">{l.costPrice != null ? formatCurrency(l.costPrice) : "—"}</TableCell>
