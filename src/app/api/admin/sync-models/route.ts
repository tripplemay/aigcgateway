...
export async function POST(request: Request) {
  ...
  runModelSync()
    .then((result) => {
      const failed = result?.summary?.totalFailedProviders ?? 0;
      const level = failed > 0 ? "ERROR" : "INFO";
      writeSystemLog(
        "SYNC",
        level,
        `Model sync completed (${failed} failed)`,
        JSON.parse(JSON.stringify(result)),
      );
    })
    .catch((err) => {
      console.error("[sync-models] background sync error:", err);
      writeSystemLog("SYNC", "ERROR", `Model sync failed: ${(err as Error).message}`);
    });
