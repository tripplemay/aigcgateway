 ...
 *
 * The lean probe replaces the old 3-tier (CONNECTIVITY / FORMAT / QUALITY)
 * chat(max_tokens:200) with a 1-token chat(max_tokens:1). These tests pin:
 ...
 *   (d) adapter is invoked with max_tokens:1, temperature:0, messages hi
 */
...
describe("runTextCheck lean (F-HPL-01)", () => {
  ...
  it("invokes adapter with max_tokens:1, temperature:0, single user 'hi' message", async () => {
    chatCompletionsMock.mockResolvedValueOnce({
