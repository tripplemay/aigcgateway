...
> 3. 在另一个 shell 中轮询 3099 端口确认就绪

### 4.1 首次测试 / 数据库结构变更 / 依赖变化后

在以下情况出现时，应优先执行完整初始化流程：

- 第一次启动 Codex 测试环境
- 数据库结构发生变化
- Prisma schema / migration 发生变化
- 依赖发生变化
- 本地测试环境状态明显异常
...
```
# 步骤 1：在持久 PTY 会话中前台运行（脚本会阻塞）
bash scripts/test/codex-setup.sh

# 步骤 2：在另一个 shell 中轮询等待就绪
for i in $(seq 1 40); do
  curl -sf --noproxy '*' http://localhost:3099/v1/models > /dev/null 2>&1 && echo "Ready" && break
  sleep 3
done
```
...
### 4.3 仅重启服务（已构建，无需重新 build）

如果只是服务进程退出、需要重新拉起，且代码和构建产物未变化：

```
# 在持久 PTY 会话中前台运行
bash scripts/test/codex-start.sh
```
...
- 默认优先选择更安全、更完整的初始化方式
- 即优先使用 `codex-setup.sh`

不得在环境状态不明的情况下跳过必要初始化。
