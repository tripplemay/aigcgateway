...
| `get_template` | 查看模板详情：system prompt、变量列表、版本历史 |
| `create_template` | 用自然语言或结构化定义创建模板，直接生效并记录版本 |
| `update_template` | 修改模板内容，自动创建新版本，旧版本可回滚 |
| `chat_with_template` | 使用模板调用 AI。传入模板 ID + 变量值，服务端组装 prompt 并调用 |
