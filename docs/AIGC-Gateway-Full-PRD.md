...
| MiniMax | 文本 | 通用引擎 | 直连 |
| Moonshot/Kimi | 文本 | 通用引擎 | 直连 |
| 阿里云百炼/Qwen | 文本 | 通用引擎 | 直连 |
| 阶跃星辰/StepFun | 文本 | 通用引擎 | 直连 |
...
| MiniMax | /models | AI 补充价格 |
| Moonshot/Kimi | /models | AI 补充价格 |
| 阿里云百炼/Qwen | /models | AI 补充价格 |
| 阶跃星辰/StepFun | /models | AI 补充价格 |
...
**Template 管理（8，多步编排工作流 CRUD + 公共模板）：**
...
| 级别 | 检查方式 | 成本 | 适用场景 |
|------|---------|------|---------|
| API_REACHABILITY | GET /models 端点探测 | 零 | 未纳入别名的通道、图片通道 |
| CONNECTIVITY (L1) | 发真实 chat 请求 | ~$0.00001/次 | 已纳入已启用别名的文本通道 |
| FORMAT (L2) | choices/usage/finish_reason 完整性 | 同上 | 同上 |
| QUALITY (L3) | 响应内容非空且合理 | 同上 | 同上 |

### 9.2 别名感知调度

| 通道类型 | 检查方式 | 频率 |
|---------|---------|------|
| 已纳入已启用别名 · 文本 | 全三级（L1→L2→L3） | 10 分钟 |
| 已纳入已启用别名 · 文本 · DISABLED | 全三级（恢复检测） | 30 分钟 |
| 已纳入已启用别名 · 图片 | API_REACHABILITY | 10 分钟 |
| 未纳入别名 / 别名未启用 | API_REACHABILITY | 10 分钟 |
| 通道刚被纳入已启用别名 | 立即触发 | — |
...
| 11 家 AI 服务商 API | 模型调用 | 各家不同 | 健康检查自动降级 |
...
**已实现：**
- Action/Template 完整 CRUD API + MCP Tools（25 个） — ✓
...
## 附录：后续功能演进（2026-04 追加）

### P5 公共模板

- 管理员可将 Template 标记为公共（public）
- 用户可浏览公共模板库、fork 到自己的项目中
- MCP 工具：`list_public_templates`、`fork_public_template`

### M1 别名架构

- 用户通过 ModelAlias（如 `gpt-4o`）访问模型，不直接使用 Provider/Model 名称
- 别名持有 sellPrice（面向用户定价）、capabilities、brand
- 路由：别名 → AliasModelLink → Model → Channel（按优先级选最优 ACTIVE 通道）
- sync 后 alias-classifier（LLM）自动将新模型归入别名

### 定价体系

- 数据库存储 USD，前端全站显示 CNY（× 管理员可配置汇率）
- 管理员可通过"参考定价"按钮查询 OpenRouter 市场价并回填
- 定价输入/展示全程 CNY，保存时 ÷ 汇率存 USD

### 运维增强（ADMIN-OPS+）

- 模型同步 / LLM 推断实时进度显示（Redis 轮询）
- 推断结果状态提示条（成功/警告/错误颜色区分）
- SystemLog 表记录运维操作，/admin/logs 新增系统日志 tab
- 创建 Provider 时自动创建 ProviderConfig
