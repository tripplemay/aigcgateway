...
- Project → ApiKey：一个项目可创建多个 API Key（区分环境/用途）
...
model ApiKey {
  ...
  description   String?                 // Key 用途描述
  status        ApiKeyStatus @default(ACTIVE)
  permissions   Json         @default("{}") // 粒度权限: { chatCompletion, imageGeneration, logAccess, projectInfo }
  expiresAt     DateTime?               // 过期时间（null = 永不过期）
  rateLimit     Int?                    // Key 级别 RPM 限制（null = 使用项目级默认）
  ipWhitelist   Json?        @map("ip_whitelist") // IP 白名单数组（null = 不限制）
  lastUsedAt    DateTime?
  createdAt     DateTime     @default(now())
  updatedAt     DateTime     @updatedAt
...
model ProviderConfig {
  ...
  staticModels    Json?    @map("static_models")     // 运营兜底：无 API 服务商的备用数据源（正常情况下为空）
  pricingOverrides Json?   @map("pricing_overrides")  // 运营兜底：手动覆盖入口（正常情况下为空）
  docUrls         Json?    @map("doc_urls")            // AI 文档提取：服务商官方文档/定价页面 URL 列表
  updatedAt       DateTime @updatedAt
...
model Channel {
  ...
  sellPriceLocked Boolean       @default(false) @map("sell_price_locked") // 运营手动改价后标记，同步不覆盖
